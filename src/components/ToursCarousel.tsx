...
const DETAILS = [
  { key: 'price', icon: '💵', label: 'Desde' },
  { key: 'duration', icon: '🕒', label: 'Duración' },
  { key: 'type', icon: '🌍', label: 'Tipo' },
  { key: 'difficulty', icon: '💪', label: 'Dificultad' },
  { key: 'availability', icon: '📅', label: 'Disponibilidad' },
] as const;
    ...
    <section id="tours" className="py-24 md:py-32 relative overflow-hidden bg-brand-dark">
      ...
      <div className="relative mx-auto max-w-7xl px-4">
        <Reveal className="text-center mb-16">
          <SectionBadge>Paquetes destacados</SectionBadge>
          <h2 className="section-title !text-white mt-4">Nuestros Paquetes Destacados</h2>
          <p className="section-subtitle !text-white/60">
            Toca la tarjeta para ver toda la información del tour
          </p>
        </Reveal>
        ...
        >
          {TOURS.map((tour, i) => (
            <SwiperSlide key={tour.title} className="!w-[300px] md:!w-[340px]">
              <motion.article
              ...
              >
                <Link
                  ...
                  aria-label={`Ver información de ${tour.title}`}
                >
                  ...
                  {/* Degradado profesional para legibilidad */}
