...
export default function TourDetail({ tour }: { tour: TourPage }) {
  ...
  const gallery = (filtered.length >= 3 ? filtered : GALLERY_FALLBACKS.filter((i) => i !== tour.heroImage))
    .slice(0, 8);
    ...
    <>
      ...
      <section className="bg-brand-teal border-y border-brand-accent/20">
        <div className="mx-auto max-w-6xl px-4 py-5 flex flex-wrap gap-x-8 gap-y-3 justify-center text-sm text-white">
          {tour.price && <InfoPill icon="💵" label="Desde" value={tour.price} />}
          {tour.duration && <InfoPill icon="🕒" label="Duración" value={tour.duration} />}
          {tour.tourType && <InfoPill icon="🌍" label="Tipo" value={tour.tourType} />}
          {tour.difficulty && <InfoPill icon="💪" label="Dificultad" value={tour.difficulty} />}
          <InfoPill icon="📅" label="Disponibilidad" value="Todo el año" />
        </div>
      </section>
      ...
      {gallery.length >= 3 && (
        <section className="bg-brand-dark py-4 overflow-hidden">
          <div className="flex gap-3 px-4 overflow-x-auto scrollbar-hide">
            {gallery.map((img, i) => (
              <GalleryThumb key={i} src={img} alt={`${tour.title} — foto ${i + 1}`} onClick={() => setLightboxIndex(i)} />
            ))}
          </div>
        </section>
      )}
      ...
      <div className="mx-auto max-w-6xl px-4 py-16">
        <div className="grid gap-12 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-14">
            ...
            {(tour.intro.length > 0 || tour.detailParagraphs.length > 0) && (
              <Reveal>
                <SectionHeading title="Detalles del paquete turístico" />
            ...
            {tour.highlights.length > 0 && (
              <Reveal delay={0.05}>
                <SectionHeading title="Lo que vivirás" />
            ...
            {tour.itinerary.length > 0 && (
              <Reveal delay={0.1}>
                <SectionHeading title="Itinerario" />
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {tour.includes.length > 0 && (
                <Reveal delay={0.15}>
                  <div className="rounded-3xl bg-green-50 border border-green-100 p-6 h-full">
                    <h3 className="font-heading text-xl font-bold text-green-800 mb-4 flex items-center gap-2">
                      ✅ Incluye
                    </h3>
              ...
              {tour.excludes.length > 0 && (
                <Reveal delay={0.2}>
                  <div className="rounded-3xl bg-red-50 border border-red-100 p-6 h-full">
                    <h3 className="font-heading text-xl font-bold text-red-800 mb-4 flex items-center gap-2">
                      ❌ No incluye
                    </h3>
                    <ul className="space-y-2">
            ...
            {tour.recommendations.length > 0 && (
              <Reveal delay={0.25}>
                <SectionHeading title="Recomendaciones" />
                <ul className="space-y-3">
                  {tour.recommendations.map((item, i) => (
                    <li key={i} className="flex gap-3 rounded-xl bg-amber-50 border border-amber-100 p-4 text-sm text-amber-900">
                      <span>💡</span>{item}
                    </li>
                  ))}
                </ul>
              ...
              <Reveal delay={0.3}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {gallery.map((img, i) => (
                    ...
                    >
                      <Image src={img} alt={`${tour.title} — foto ${i + 1}`} fill className="object-cover group-hover:scale-105 transition duration-500" sizes="25vw" />
            ...
            <div className="sticky top-32 space-y-5">
              <Reveal>
                <div className="rounded-3xl bg-gradient-to-br from-brand-dark via-brand-teal to-brand-accent p-6 text-white shadow-2xl">
                  <p className="text-xs uppercase tracking-[0.2em] text-brand-gold mb-2">Reserva ahora</p>
                  <h3 className="font-heading text-xl font-bold mb-3">¿Te interesa este tour?</h3>
                  {tour.price && (
        ...
        <Reveal className="mt-20">
          <SectionHeading title="Lo que dicen nuestros clientes" center />
          <div className="flex gap-4 overflow-x-auto pb-4">
            {TESTIMONIALS.slice(0, 4).map((img, i) => (
              <div key={i} className="relative h-64 w-44 shrink-0 rounded-2xl overflow-hidden shadow-lg">
                <Image src={img} alt="Testimonio" fill className="object-contain bg-white" />
              </div>
            ))}
          </div>
        </Reveal>
        ...
        {related.length > 0 && (
          <Reveal className="mt-16">
            <SectionHeading title="Tours relacionados" center />
        ...
        <div className="mt-12 text-center">
          <Link href="/" className="text-brand-accent hover:underline text-sm">← Volver al inicio</Link>
        </div>
      </div>
...
function GalleryThumb({ src, alt, onClick }: { src: string; alt: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
